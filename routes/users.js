...
router.post('/register', function(req, res){
	...
	//Validar campos
	req.checkBody('name', 'Campo Obrigatório').notEmpty();
	req.checkBody('email', 'Campo Obrigatório').notEmpty();
	req.checkBody('email', 'Campo invalido').isEmail();
	req.checkBody('username', 'Campo Obrigatório').notEmpty();
	req.checkBody('password', 'Campo Obrigatório').notEmpty();
	req.checkBody('password2', 'Senhas não concidem').equals(req.body.password);
	...
	if(errors){
		...
		req.flash('success_msg', 'Voce foi registrado com sucesso');
